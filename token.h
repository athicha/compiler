 ...
 * Athicha Srivirote
 * ats219
 * Program Description: Token.h contains all fields of token
 *                      
 * Compiler
 */
#include <string>
...
class Token {
private: 
  int type; // hold token type
  string lexeme; // hold string read
  int line; // hold input file line number
  int pos;  // hold the character position, position of the first character in the token
...
public:
  ...
  Token(int intValue, string input, int lineNumber, int position);
  ~Token();
